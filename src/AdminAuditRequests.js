...
export default function AdminAuditRequests() {
  ...
  const loadAll = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/admin/audits`);
  ...
  const updateStatus = async (auditId, status) => {
    ...
    await fetch(`${API_BASE}/api/admin/audits/${auditId}/status`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status, adminComment, assignedAuditor }),
    });
